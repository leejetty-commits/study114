...
const samplePresets = read('preview/home-ui/src/inquiry-settings/sample-presets.js');
...
assert(sampleUi.includes('home-body home-body--with-promo'), 'sample ui: home-body with-promo context');
...
assert(!/inq-sample__home-context[\s\S]{0,400}\.expo-hcard\s*\{[^}]*width:\s*100%/.test(css), 'css: does not stretch hcard to 100%');
...
assert(htmlOrder.includes('inq-sample__home-context'), 'sample: home-body context');
assert(htmlOrder.includes('guest-browse-lists'), 'sample: guest-browse-lists wrapper');
