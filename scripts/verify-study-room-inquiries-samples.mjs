/**
 * 공부방 쪽지설정 — 홈 BASIC 카드 샘플 · 가상데이터 · 쪽지 화살표
 */
...
assert(screens.includes('p20-inq-sample__arrow'), 'screens: arrow marker');
assert(screens.includes('renderInquirySampleCard(true)'), 'screens: open sample');
assert(screens.includes('renderInquirySampleCard(false)'), 'screens: closed sample');
...
assert(screens.indexOf('data-p20-inquiry-save') < screens.indexOf('p20-inquiries-section--samples'), 'screens: save before samples');
...
assert(css.includes('--p20-listing-w'), 'css: home listing width token');
assert(css.includes('.p20-inq-sample__arrow'), 'css: arrow');
assert(css.includes('flex-direction: column'), 'css: vertical samples');
assert(!/p20-inq-sample__card[\s\S]{0,180}transform:\s*scale/.test(css), 'css: sample card is not scaled');
assert(!/p20-inq-sample__card[\s\S]{0,400}grid-template-columns:\s*[0-9.]+rem/.test(css), 'css: sample does not override BASIC thumb columns');
