...
final class PasswordResetMailTemplate
{
     ...
     */
    public static function build(string $resetUrl, int $ttlMinutes = 30): array
    {
        $ttlMinutes = max(1, $ttlMinutes);
        $plain = self::plainBody($resetUrl, $ttlMinutes);
        $html = self::htmlBody($resetUrl, $ttlMinutes);
    ...
    private static function plainBody(string $resetUrl, int $ttlMinutes): string
    {
        return "안녕하세요.\n"
    ...
    private static function htmlBody(string $resetUrl, int $ttlMinutes): string
    {
