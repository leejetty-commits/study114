...
final class ContentSchemaMigrateService
{
    ...
    private function seedOperationalPosts(): string
    {
        ...
        foreach ($this->operationalPostRows() as $row) {
            ...
            if ($existingId) {
                // FAQ·안전과외 시드 문구는 배포 시 최신 카피로 갱신
                if (
                    ($row['board_key'] === 'faq' && preg_match('/^faq-\d+$/', (string) $row['post_key']))
                    || $row['board_key'] === 'safe-guide'
                ) {
    ...
    private function operationalPostRows(): array
    {
        return [
            ...
            [
                ...
                'description' => '**고객센터** — 이메일/문의 폼을 이용합니다. 쪽지함과는 별개입니다.',
                'category_id' => 'support',
                'meta_json' => json_encode(['answer' => '**고객센터** — 이메일/문의 폼을 이용합니다. 쪽지함과는 별개입니다.', 'sortOrder' => 20], JSON_UNESCAPED_UNICODE),
                'created_at' => date('Y-m-d H:i:s'),
            ],
            [
                'board_key' => 'faq',
                ...
                'description' => '없습니다. 대금·연락 중개·보증은 우동공과에서 제공하지 않는 서비스입니다.',
                'category_id' => 'safety',
                'meta_json' => json_encode(['answer' => '없습니다. 대금·연락 중개·보증은 우동공과에서 제공하지 않는 서비스입니다.', 'sortOrder' => 30], JSON_UNESCAPED_UNICODE),
                'created_at' => date('Y-m-d H:i:s'),
            ],
            [
                'board_key' => 'faq',
            ...
            [
                ...
                'status' => 'published',
                'title' => '프라임/픽은 무엇인가요?',
                'description' => '동네 노출 기간형 포지션 상품입니다. Hot·추천 등 광고배지는 포지션에 종속됩니다.',
                'category_id' => 'billing',
                'meta_json' => json_encode(['answer' => '동네 노출 기간형 포지션 상품입니다. Hot·추천 등 광고배지는 포지션에 종속됩니다.', 'sortOrder' => 50], JSON_UNESCAPED_UNICODE),
                'created_at' => date('Y-m-d H:i:s'),
            ],
            [
                'board_key' => 'faq',
    ...
    private function guidePostRows(): array
    {
        $guides = [
            ...
            ], null, ['prepay', 'first-meeting', 'parent-check']],
            ['provider-check', '공급자(공부방·과외) 체크리스트', 'secondary', '공급자', [
                ...
                ['label' => '제출자료·공개 상태 확인', 'hint' => '마이페이지 제출자료와 학부모에게 보이는 범위 확인'],
            ...
            ], ['parent-check', 'privacy', 'safe-what']],
            ...
            ], ['first-meeting', 'provider-check', 'parent-check']],
        ];

        $rows = [];
        foreach ($guides as [$slug, $title, $priority, $audience, $body, $checklist, $related]) {
            $meta = [
                'slug' => $slug,
                'priority' => $priority,
                'audience' => $audience,
