...
import { getListTabFromPath, tabLabel, tabPath, parseThreadId } from './router.js';
...
function renderList(tab) {
  ...
  const rows =
    threads.length === 0
      ...
      : threads
          .map(
            (t) => `
      <a href="#/messages/thread/${t.id}" class="msg-row${t.unread ? ' is-unread' : ''}" data-msg-nav="/messages/thread/${t.id}">
  ...
  const demoHint =
    isProviderRole(role) && !canProviderMemoToStudent(role)
      ? `<p class="msg-note msg-note--warn">무료 공급자: 학생 메모 시도 → P16-04 · 툴바/학생 상세에서 유무료 전환</p>`
      : '';
...
function renderEmptyList(role) {
  ...
  if (isProviderRole(role) && !canProviderMemoToStudent(role)) {
    return `<p class="msg-empty">학생에게 메모하려면 유료등록이 필요합니다. (P16-04)</p>`;
  }
...
function renderThread(threadId) {
  ...
  if (!thread) {
    return `<section class="msg-panel msg-empty">대화를 찾을 수 없습니다. <a href="#/messages/inbox" data-msg-nav="/messages/inbox">목록으로</a></section>`;
  }
  ...
  return `
    <section class="msg-panel msg-thread">
      <div class="msg-thread__bar">
        <a href="#/messages/inbox" class="msg-back" data-msg-nav="/messages/inbox">← 목록</a>
