...
export const GUIDE_NAV_ITEMS = [
  ...
  { id: 'register', label: '등록·노출', path: '/guide/register' },
...
export const GUIDE_HOME_ENTRIES = [
  ...
  },
  {
    ...
    label: 'Register',
    title: '등록·노출',
    desc: '공부방·과외쌤 등록을 시작한다면, 필수정보를 입력하면 기본 노출이 시작되는 순서를 안내해요.',
    cta: '등록 순서 보기',
    path: '/guide/register',
    icon: I.filePen,
  },
...
export const GUIDE_ROLE_CARDS = [
  ...
  {
    eyebrow: '공부방',
    title: '공부방 정보를 등록하고 노출·운영하려는 분',
    body: '등록방법에서 기본등록과 상세등록의 차이를 먼저 확인하고, 이후 유료상품과 연결 여부를 판단하세요.',
    ctas: [
      { label: '등록·노출 보기', path: '/guide/register' },
  ...
  },
  {
    ...
    body: '등록방법으로 노출 흐름을 이해하고, 찜·비교·쪽지 페이지에서 첫 연락 구조를 확인해보세요.',
    ctas: [
      { label: '등록·노출 보기', path: '/guide/register' },
...
export const GUIDE_FLOW_SUMMARY = [
  ...
  {
    title: '등록하는 사람',
    steps: ['가입', '필수정보 입력', '기본 노출', '상세등록(보완)', '유료 연결'],
  },
  {
    title: '도움이 필요할 때',
    steps: [
...
export const GUIDE_PAGES = {
  start: {
    ...
    flow: [
      ...
      { label: '상세 확인', caption: '소개·수업·노출된 신뢰정보', icon: I.userCheck },
    ...
    steps: [
      ...
      },
      {
        ...
        body: '리스트만으로 부족하다면 상세에서 소개, 수업 방식, 노출된 신뢰정보를 더 확인해보세요.',
        icon: I.userCheck,
      },
      {
  ...
  register: {
    ...
    summary:
      '가입 필수정보를 입력하면 기본 노출이 시작됩니다. 상세등록은 카드와 상세 페이지에 보여줄 추가 정보를 보완하는 단계이며, 노출을 위한 운영자 심사·승인 절차는 없습니다.',
    principles: [
      '공통가입 = 계정 생성용 최소 단계',
      '기본등록 = 가입 필수정보 입력. 입력하면 기본 노출이 시작됩니다',
      '상세등록 = 카드·상세에 보여줄 추가 정보 보완. 기본 노출의 필수 조건이 아닙니다',
      '기본 노출 = 필수정보 입력 후. 운영자 심사·승인 없음',
    ...
    flow: [
      ...
      { label: '기본등록', caption: '필수정보 입력', icon: I.filePen },
      { label: '기본 노출', caption: '검색·리스트에 보이기', icon: I.sparkles },
      { label: '상세등록', caption: '카드·상세 추가 정보 보완', icon: I.clipboard },
    ...
    steps: [
      ...
      {
        title: '필수정보를 입력하면 기본 노출이 시작됩니다',
        body: '기본등록은 가입 필수정보를 입력하는 단계입니다. 이 정보를 입력하면 기본 노출이 시작되며, 운영자 심사·승인을 기다리지 않습니다.',
        icon: I.filePen,
      },
      {
        title: '상세등록은 추가 정보를 보완하는 단계입니다',
        body: '상세등록은 카드와 상세 페이지에 보여줄 추가 정보를 보완합니다. 기본 노출의 필수 조건은 아닙니다.',
        icon: I.clipboard,
      },
      {
        title: '노출을 위한 심사·승인은 없습니다',
        body: '가입 필수정보를 입력하면 기본 노출이 시작됩니다. 상세를 전부 채워야 목록에 나온다는 조건은 없고, 운영자 심사·승인 절차도 없습니다.',
        icon: I.sparkles,
      },
      {
    ...
    detailGroups: [
      {
        ...
        lead: '공부방 필수정보를 입력하면 기본 노출이 시작됩니다. 소개와 운영 정보는 상세등록에서 카드·상세에 보완하며, 기본 노출의 필수 조건은 아닙니다.',
        columns: [
          {
            title: '기본등록에서 하는 일',
      ...
      },
      {
        ...
        lead: '활동 지역과 주력과목 등 필수정보를 입력하면 기본 노출이 시작됩니다. 학부모가 볼 추가 정보는 상세등록에서 보완합니다.',
        columns: [
          {
            title: '기본등록에서 하는 일',
    ...
    faq: [
      {
        q: '가입 필수정보를 입력하면 기본 노출이 시작되나요?',
        a: `네.
가입 필수정보를 입력하면 기본 노출이 시작됩니다.
상세등록은 카드와 상세 페이지에 보여줄 추가 정보를 보완하는 단계입니다.
노출을 위한 운영자 심사·승인 절차는 없습니다.

회원가입은 계정을 만드는 단계이고,
역할에 맞는 필수정보를 입력하면 검색·리스트 기본 노출이 시작됩니다.
상세를 전부 채워야 목록에 나오는 것은 아닙니다.

즉,
회원가입만 완료한 상태 = 아직 필수정보 입력 전
필수정보 입력 완료 = 기본 노출 시작
상세등록 = 카드·상세 추가 정보 보완
이라고 보시면 돼요.`,
      },
      {
        ...
        a: `가장 쉽게 말하면:
기본등록은 가입 필수정보를 입력하는 단계이고, 입력하면 기본 노출이 시작됩니다.
상세등록은 카드와 상세 페이지에 보여줄 추가 정보를 보완하는 단계입니다.
기본 노출의 필수 조건은 아닙니다.

기본등록에서는 너무 많은 정보를 한 번에 받지 않아요.
노출에 필요한 필수정보만 먼저 입력하게 되어 있어요.
...
한 줄로 정리하면:
기본등록은 “필수정보 입력 후 기본 노출”,
상세등록은 “카드·상세에 보여줄 추가 정보 보완”이에요.`,
      },
      {
...
정도로 이해하면 가장 편합니다.`,
      ...
      {
        q: '기본 노출 전에 어떤 정보를 먼저 준비해야 하나요?',
        a: `기본 노출 전에는 복잡하게 생각하지 마시고,
가입 필수정보를 먼저 준비한다고 생각하시면 돼요.
필수정보를 입력하면 기본 노출이 시작되고, 나머지 상세는 나중에 보완해도 됩니다.
...
우동공과의 흐름은 처음엔 가볍게 시작하고, 상세등록에서 점점 보완하는 구조에 더 가깝습니다.`,
      },
      {
        ...
        a: `유료상품은
등록 내용을 다 입력하는 단계가 아니라, 기본 노출 이후에 노출이나 접근을 더 강화하는 단계예요.

즉 순서상으로는 보통:
회원가입 → 필수정보 입력 → 기본 노출 → 상세등록(보완) → 필요 시 유료상품 연결
이 흐름으로 이해하시면 됩니다.
...
쉽게 말하면:
필수정보 입력 전: 아직 기본 노출 전
필수정보 입력 후: 기본 노출 시작. 상세등록은 추가 정보 보완
...
유료상품부터 보기보다, 먼저 기본등록과 상세등록을 차근차근 끝내는 것이 더 중요해요.`,
    ...
    footerNote: [
      '가입 필수정보를 입력하면 기본 노출이 시작됩니다. 상세등록은 카드와 상세의 추가 정보 보완이며, 노출을 위한 운영자 심사·승인 절차는 없습니다.',
  ...
  compare: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: '찜이나 비교만으로 결정하기 어렵다면 상세에서 수업 방식, 소개, 노출된 신뢰정보, 접촉 가능 여부를 다시 확인하세요.',
        icon: I.userCheck,
      },
      {
...
예를 들면 지역, 과목, 대상 학년, 수업 방식, 가격대, 소개문 분위기, 노출된 신뢰 정보를 한 화면에서 정리해 볼 수 있어요.
...
실제 연락 전에는 상세에서 소개, 수업 방식, 노출된 정보, 내게 중요한 조건을 다시 확인해보는 것이 좋아요.
...
한 줄로 정리하면: 찜은 저장, 비교는 판단, 쪽지는 첫 연락이에요.`,
  ...
  safe: {
    ...
    flowLead: '먼저 전체 흐름을 차트로 보고, 아래 단계별 설명에서 더 자세히 확인할 수 있습니다.',
    flow: [
      { label: '노출 정보 확인', caption: '제출자료·소개를 직접 비교', icon: I.shield },
    ...
    steps: [
      {
        title: '노출된 신뢰정보를 먼저 확인하세요',
        body: '소개, 제출자료, 노출된 경력 정보 등 상대가 보여 둔 정보를 먼저 차분히 확인해보세요.',
        icon: I.shield,
      },
      {
    ...
    warnings: [
      ...
      '검증·인증·보증처럼 오해될 수 있는 표현 대신 노출된 신뢰정보를 직접 확인하세요.',
