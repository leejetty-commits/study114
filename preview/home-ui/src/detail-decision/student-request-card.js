...
import { getStudentProtectedVisibility, PAID_GATE_MESSAGE } from '../student-visibility.js';
  ...
  if (!visible) {
    return `
      <div class="p24-protected p24-protected--blocked">
        ...
        <p class="p24-protected__msg">${PAID_GATE_MESSAGE}</p>
      </div>`;
  }
  return `
...
export function renderStudentRequestBody(student, viewer) {
  ...
  return `
    ...
    <section class="p24-section">
      ...
      ${
        viewer === 'tutor'
          ? `<p class="p24-sub-hint">공급자 구독: <strong>${sub === 'paid' ? '유료' : '무료'}</strong>
        · 요청문 ${VISIBILITY_LABELS[student.request_summary_visibility] || '비공개'}
        · 특이요청 ${VISIBILITY_LABELS[student.special_request_visibility] || '비공개'}</p>`
          : ''
      }
    </section>`;
}
