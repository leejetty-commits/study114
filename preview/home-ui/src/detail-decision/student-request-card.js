...
function renderProtectedBlock(label, content, visible, visibility, studentId) {
  if (visible) {
    return `
...
export function renderStudentRequestBody(student, viewer) {
  const vis = getStudentProtectedVisibility(student);
  ...
  return `
    ...
    <section class="p24-section">
      ...
      ${renderProtectedBlock('요청문', student.request_summary, vis.requestSummary, student.request_summary_visibility || 'private', studentId)}
      ${renderProtectedBlock('특이요청사항', student.special_request_note, vis.specialRequest, student.special_request_visibility || 'private', studentId)}
      ${
        ...
        · 열람권 ${getRequestViewGateState().ticketsRemaining}회</p>`
          : ''
      }
    </section>`;
}
