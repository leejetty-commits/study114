...
### 2-0. 2026-08-21 카드 이모티콘/아이콘 최신 잠금

이 절을 11장의 **최신 기준**으로 본다. 아래와 기존 표·샘플이 충돌하면 **이 절을 우선**한다.  
코드 SSOT: `preview/home-ui/src/card-visual.js` · 검증: `npm run verify:card-visual` (repo root)

| 층 | 내용 |
|----|------|
| **기능** | 찜 / 비교 / 쪽지 |
| **통계** | 추천 수 / 후기 개수 (후기 0이면 숨김) |
| **제거** | 좋아요 |
| **자동부여** | New — 공부방·과외쌤 공통, 신규 1주, **판매 아님** |
| **무료 신뢰** | 교육청 등록 · 사업자 · 졸업 · 경력 · 증빙/검수형 |
| **유료(공부방)** | Hot / **단과** |
| **유료(과외쌤)** | Hot / 쪽집게(`jjokjipge`) / SKY |

표현 원칙:
- 추천·후기는 **통계**이지 유료 광고배지가 아니다.
- 공부방 **「전문」은 아이콘/배지 금지** — 헤드라인 카피만. 유료 아이콘은 **단과**.
- 기능 / 통계 / 신뢰 / 유료 주목을 **같은 UI 의미로 섞지 않는다**.
