...
3. 사용자는 **사이트 표시명**만 마이페이지에서 수정한다. 로그인 메일은 헤더·표시명 fallback·마이페이지 보조줄에 올리지 않는다.
4. auth email 자체 변경은 **후속 검토** (계정 찾기·비번 재설정·중복·관리자 목록 영향). 본인 계정설정에서 로그인 식별자를 보는 것은 시장 노출이 아니다.
