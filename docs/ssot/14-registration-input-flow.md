...
| 3 | 역할 선택 | 학부모/학생 · 공부방 · 과외쌤 | 최초 흐름은 1역할 기준 · 이때 계정 생성 |
| 3b | 이메일 확인 대기 | 확인 메일 · 링크 클릭 | **이 단계 전은 가입 완료 아님** |
| 4 | 역할별 기본등록 | 검색·비교 **핵심축** 필수 수집 | 확인 후에만 |
| 5 | 가입 완료 | 축하 + 기본 노출 안내 | Local List 자격 |
| 6 | 상세등록 유도 | Prime/Pick 연결 CTA | 상세 완료 후 광고 상품 자격 |

auth-ui 라우트: `#/signup/terms` → `form` → `role` → **`verify-email`** → `basic` → `complete`

**가입 완료:** 이메일 확인(`email_verified_at`) 후. 계정 생성만으로는 완료가 아니다.
