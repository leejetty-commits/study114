...
| P19-05 공개설정 | P20-05 **쪽지와 문의** |
| P19-06 숨김·삭제 | P20-06 **논리 ID** (UI는 허브·별도 후순위) |
...
| 4 | **미리보기·공개** | P20-04 |
| 5 | **쪽지와 문의** | P20-05 |
| 6 | **상품연결** | P15-09 · 18장 (후순위 · P20-05와 분리) |
...
**등록정보 필드가 아니라 운영 스위치.** 공부방 원장이 **지금 쪽지 문의를 받을지**만 관리한다.  
**기본정보·상세정보·공개 전환(P20-04)은 `inquiry_status`를 변경하지 않는다.**

> **공개 / 쪽지 / 유료(홍보 포지션)는 별개 축.** `published`여도 `inquiry_status`가 닫혀 있으면 학부모 카드 CTA는 `지금은 쪽지 안 받음`이다.

| code | 운영자 UI | 학부모 카드 CTA | 쪽지 작성 |
|------|-----------|-----------------|-----------|
| `open` | 쪽지 받는 중 ON | `쪽지하기` | 허용 |
| `capacity_full` | OFF · 정원 마감 | `지금은 쪽지 안 받음` (+ 정원 마감) | 차단 |
...
## 7. P20-05 쪽지와 문의 · P20-06

### 7-1. P20-05

**화면명:** `쪽지와 문의` — **지금 쪽지 문의를 받을지**만 관리하는 단일 운영 화면.

**정본 경로:** `#/mypage/registrations/study-rooms/:id/inquiries`  
**레거시:** `.../exposure` → `.../inquiries` 리다이렉트 (route key `exposure` 재사용 ✕)

**블록 (A~F만):**

1. **상단 상태 요약** — 받는 중/안 받음 · 카드 버튼 문구 · 연락처 확인
2. **메인 스위치** — `쪽지 받는 중` ON/OFF
3. **OFF 사유** — `정원 마감` · `잠시 쉼` (OFF 저장 시 필수)
4. **기본 연락처 내부 확인** — 확인 완료/필요 · 비공개 고지
5. **카드 미리보기** — `쪽지하기` / `지금은 쪽지 안 받음`
6. **저장 CTA** — `저장하기`

**P20-05에서 제외 (이번 리뉴얼):** 검색/비교/지도 매트릭스 · 홍보 포지션 · 상품 CTA · 숨김/삭제 · 과외 권한 설명.

**금지 표현:** 노출·상담 · 상담 수용 · 쪽지 활성화 · 대기 문의 가능(1차 UI).

**휴대폰 게이트:** `쪽지 받는 중` ON 저장 시도 시 `phone_verified_at` 확인 · 실패 시 OFF·기존 사유 유지.
...
| `.../study-rooms/:id/inquiries` | P20-05 |
| `.../study-rooms/:id/exposure` | (레거시 → `inquiries` 리다이렉트) |
